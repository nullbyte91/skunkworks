        ...
        for cycle in range(0, MAX_CYCLES):
            ...
            for episode in range(MAX_EPISODES):
                ...
                while np.linalg.norm(achieved_goal - desired_goal) <= 0.05:
                    # if black thing and red is very close to each other, reset                    
                    env_dict = env.reset()
                    state = env_dict["observation"]
                    achieved_goal = env_dict["achieved_goal"]
                    desired_goal = env_dict["desired_goal"]
            ...
            agent.store(mb)#adds all episodes as minibatches to agent's memory. 
            #And does some normalizations and syncs
